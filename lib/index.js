...
DataStore.prototype.applyFieldsFilterToResults = function (fields, documents) {
  ...
  let normalisedDocuments = Array.isArray(documents) ? documents : [documents]
  let flattenedProjection = Array.isArray(fields)
    ...
    : fields
  let projection = this.convertFieldsProjection(flattenedProjection)
  let isExclusion = Object.keys(projection).some(field => {
    return projection[field] === 0
  })
  let filteredDocuments = normalisedDocuments.map(document => {
      ...
      indexes.forEach((index, idx) => {
        if (
          ...
          if (index.options && index.options.unique) {
            let uniqIdx = collection.ensureUniqueIndex(
