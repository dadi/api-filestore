...
DataStore.prototype.getFieldOrParentSchema = function (key, schema) {
  ...
  //  console.log('keyOrParent:', keyOrParent)
...
DataStore.prototype.prepareQuery = function (query, schema) {
  ...
  //     var field = this.getFieldOrParentSchema(key, schema)
  //     var settings = field ? field.settings : {}
...
DataStore.prototype.find = function (query, collection, options, schema) {
  ...
  return new Promise((resolve, reject) => {
    this.getCollection(collection).then(collection => {
      ...
      var baseResultset = collection.chain().find(query)
      var branchedResultset = baseResultset.branch()

      // count of records matching the filter
      var count = branchedResultset.count()
      ...
      _.each(indexes, (index, idx) => {
        if (Object.keys(index.keys).length === 1 && Object.keys(index.keys)[0] === '_id') {
          ...
          if (index.options.unique) {
            var uniqIdx = collection.ensureUniqueIndex(Object.keys(index.keys)[0])
...
DataStore.prototype.getIndexes = function (collectionName) {
  return new Promise((resolve, reject) => {
    this.getCollection(collectionName).then(collection => {
      var indexes = []
      _.each(Object.keys(collection.binaryIndices), key => {
        indexes.push({ name: key })
      })

      _.each(Object.keys(collection.constraints.unique), key => {
...
DataStore.prototype.dropDatabase = function (collectionName) {
  ...
  return new Promise((resolve, reject) => {
    ...
    _.each(this.database.collections, (collection, idx) => {
      // if (collectionName) {
      ...
      if (idx === this.database.collections.length - 1) {
        setTimeout(() => {
          return resolve()
        }, 500)
      }
